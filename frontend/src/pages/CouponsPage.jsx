...
export default function CouponsPage() {
  ...
  const openEdit = (coupon) => {
    ...
    setForm({
      ...
      same_item_required: coupon.same_item_required === true || coupon.offer_type === "bogo",
    ...
    // B11 fix: clean up any existing overlap between eligible and excluded on edit load
    const eligibleIds = coupon.eligible_food_ids || [];
    const excludedIds = coupon.excluded_item_ids || [];
    if (eligibleIds.length > 0 && excludedIds.length > 0) {
      const overlap = excludedIds.filter(id => eligibleIds.includes(id));
      if (overlap.length > 0) {
        setForm(p => ({ ...p, excluded_item_ids: p.excluded_item_ids.filter(id => !eligibleIds.includes(id)) }));
      }
    }
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (selectedType === "every_nth") {
        ...
        payload.eligible_category_ids = selectedCats.map(c => c.id);
        payload.eligible_category_names = selectedCats.map(c => c.name);
        // B11 fix: strip any overlap at save time (defensive, toggle handlers should already prevent)
        const safeExcluded = form.excluded_item_ids.filter(id => !form.eligible_food_ids.includes(id));
        payload.excluded_item_ids = safeExcluded.length > 0 ? safeExcluded : null;
        payload.max_applications = form.max_applications !== "" ? parseInt(form.max_applications, 10) : null;
        payload.allow_repeat = form.allow_repeat !== false;
        payload.apply_to_cheapest_item = !!form.apply_to_cheapest_item;
        payload.apply_to_highest_item = !!form.apply_to_highest_item;
  ...
  // B11 fix: toggling eligible auto-removes from excluded and vice versa
  const toggleFoodId = (fid) => setForm(p => {
    const isRemoving = p.eligible_food_ids.includes(fid);
    return { ...p, eligible_food_ids: isRemoving ? p.eligible_food_ids.filter(x => x !== fid) : [...p.eligible_food_ids, fid], excluded_item_ids: isRemoving ? p.excluded_item_ids : p.excluded_item_ids.filter(x => x !== fid) };
  });
  const toggleExcludedFoodId = (fid) => setForm(p => {
    const isRemoving = p.excluded_item_ids.includes(fid);
    return { ...p, excluded_item_ids: isRemoving ? p.excluded_item_ids.filter(x => x !== fid) : [...p.excluded_item_ids, fid], eligible_food_ids: isRemoving ? p.eligible_food_ids : p.eligible_food_ids.filter(x => x !== fid) };
  });
            ...
            <div className="px-6 py-5">
              ...
              {selectedType && (
                <form id="coupon-form" onSubmit={handleSubmit} className="space-y-6">
                  ...
                  {menuError && (form.discount_scope === "item" || form.discount_scope === "category") && (
                  ...
                  {selectedType === "every_nth" && (
                    <>
                      ...
                      <div className="space-y-4">
                        ...
                        <ItemSelector items={menuItems.filter(i => !form.excluded_item_ids.includes(i.food_id))} selected={form.eligible_food_ids} onToggle={toggleFoodId} loading={menuLoading} label="Eligible Menu Items" />
                      ...
                      <div className="space-y-4" data-testid="excluded-items-section">
                        ...
                        <ItemSelector items={menuItems.filter(i => !form.eligible_food_ids.includes(i.food_id))} selected={form.excluded_item_ids} onToggle={toggleExcludedFoodId} loading={menuLoading} label="Excluded Items" />
                      </div>
                      <Separator className="bg-gray-100" />
                      <div className="space-y-4">
                    ...
                    <div className="grid grid-cols-2 gap-3">
                      ...
                      <div>
                        ...
                        <Input type="number" value={form.per_user_limit} onChange={e => setForm({ ...form, per_user_limit: e.target.value })}
                          placeholder="Unlimited" className="h-11 rounded-xl bg-gray-50/50" min="1" data-testid="per-user-limit" />
                      </div>
                    </div>
                    <div>
