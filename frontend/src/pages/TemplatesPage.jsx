                                            ...
                                            <div className="flex items-center justify-between">
                                                ...
                                                <div className="flex rounded-lg border bg-white overflow-hidden">
                                                    {isCouponVar || isCouponPickMode ? (
                                                        <>
                                                            <button type="button" onClick={() => { setVariableMappingModes(prev => ({...prev, [variable]: "coupon_pick"})); if (couponSummary.length === 0) fetchCouponSummary(); if (selectedCouponId && mappedVarInfo?.key) { const cpn = couponSummary.find(c => c.id === selectedCouponId); if (cpn) { const cf = mappedVarInfo.key.replace("coupon_", ""); setVariableMappings(prev => ({...prev, [variable]: `coupon:${cpn.id}:${cf}`})); } } }} className={`px-3 py-1 text-xs font-medium transition-colors ${isCouponPickMode ? "bg-[#F26B33] text-white" : "bg-white text-gray-600 hover:bg-gray-100"}`} data-testid={`var-mode-coupon-pick-${variable}`}>Pick Coupon</button>
                                                            <button type="button" onClick={() => { setVariableMappingModes(prev => ({...prev, [variable]: "text"})); setVariableMappings(prev => ({...prev, [variable]: ""})); }} className={`px-3 py-1 text-xs font-medium transition-colors ${currentMode === "text" ? "bg-[#F26B33] text-white" : "bg-white text-gray-600 hover:bg-gray-100"}`} data-testid={`var-mode-text-${variable}`}>Custom Text</button>
                                                        </>
                                                    ) : (
                                                        <>
                                                            <button type="button" onClick={() => setVariableMappingModes(prev => ({...prev, [variable]: "map"}))} className={`px-3 py-1 text-xs font-medium transition-colors ${currentMode === "map" ? "bg-[#F26B33] text-white" : "bg-white text-gray-600 hover:bg-gray-100"}`} data-testid={`var-mode-map-${variable}`}>Map to Field</button>
                                                            <button type="button" onClick={() => setVariableMappingModes(prev => ({...prev, [variable]: "text"}))} className={`px-3 py-1 text-xs font-medium transition-colors ${currentMode === "text" ? "bg-[#F26B33] text-white" : "bg-white text-gray-600 hover:bg-gray-100"}`} data-testid={`var-mode-text-${variable}`}>Custom Text</button>
                                                        </>
                                                    )}
                                                </div>
                                            </div>
                                            {isCouponPickMode && pickedCoupon ? (
                                                ...
                                                <div className="space-y-2" data-testid={`coupon-picker-${variable}`}>
                                                    {couponSummaryLoading ? (
                                                        ...
                                                        <div className="bg-gray-100 rounded-lg p-3 text-center"><p className="text-sm text-gray-500">No coupons found. Create a coupon first.</p></div>
                                                    ) : (
                                                        <>
                                                            ...
                                                            <div className="max-h-40 overflow-y-auto space-y-1.5">
                                                                ...
                                                                {couponSummary.filter(c => { const q = couponSearchQuery.toLowerCase(); return !q || c.code.toLowerCase().includes(q) || (c.title || "").toLowerCase().includes(q); }).length === 0 && (
                                                ...
                                                <Select value={variableMappings[variable] || ""} onValueChange={(val) => {
                                                    setVariableMappings(prev => ({...prev, [variable]: val}));
                                                ...
                                                }}><SelectTrigger className="h-10 rounded-lg bg-white"><SelectValue placeholder="Select a field..." /></SelectTrigger><SelectContent><SelectItem value="none">-- None --</SelectItem>{availableVariables.map(field => (<SelectItem key={field.key} value={field.key}>{field.label} (e.g., {field.example})</SelectItem>))}</SelectContent></Select>
                                            )}
                                        </div>
                                        );
                                    })}
