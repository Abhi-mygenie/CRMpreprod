...
import { MessageSquare, Settings, Plus, Edit2, Trash2, Eye, EyeOff, Filter, Clock, Tag, Save, Wallet, KeyRound, Send, Loader2, Lock, AlertTriangle, Upload, ExternalLink, Phone } from "lucide-react";
...
export default function TemplatesPage() {
    ...
    const [crmTemplatesEnabled, setCrmTemplatesEnabled] = useState(false);   // CR-061
    ...
    const handleReuploadComplete = async (data) => {
        try {
            await api.put(`/whatsapp/custom-templates/${reuploadTemplate.id}`, {
                header_handle: data.handle,
                send_media_url: data.send_media_url,
                send_media_filename: data.filename,
                header_media_mime: data.mime,
            });
            toast.success("Media re-uploaded — template can now send.");
            setReuploadTemplate(null);
            fetchCustomTemplates();
        } catch (err) {
            toast.error(err?.response?.data?.detail || "Failed to save re-uploaded media");
        }
    };
    ...
    useEffect(() => {
        const fetchData = async () => {
            ...
            try {
                ...
                setCrmTemplatesEnabled(!!res.data.crm_templates_enabled);   // CR-061
            } catch (_) {}
                                ...
                                <>
                                    <div className="flex items-center justify-between mb-3 flex-wrap gap-2">
                                        ...
                                        <div className="flex items-center gap-2">
                                            ...
                                            {/* CR-061 revised: "Add Template" always visible for all tenants; only the CRM drafts section is gated */}
                                    ...
                                    {/* Draft Templates */}
                                    {/* CR-061: entire drafts section (which houses CRM Templates + Edit entry points) is hidden for non-allowlisted tenants */}
                                    {crmTemplatesEnabled && displayDrafts.length > 0 && (
                                        <div className="mb-4">
                                                            ...
                                                            <div className="flex gap-2 mt-3 flex-wrap">
                                                                ...
                                                                {ct.needs_media_reupload && (
                                                                    <Button size="sm" variant="outline"
                                                                        ...
                                                                        onClick={() => setReuploadTemplate(ct)}
                                                ...
                                                <CardContent className="p-4">
                                                    ...
                                                    {tpl.temp_status === 1 && variables.length > 0 && (
                                                        <div className="flex flex-wrap gap-1.5 mb-2">
                                                            ...
                                                            {(tpl.buttons || []).filter(b => b.url_type === "dynamic").map((btn, idx) => {
                                                                ...
                                                                if (!btnVarMatch) return null;
                                                                const btnKey = `btn_url_{{${btnVarMatch[1]}}}`;
                                                    ...
                                                    <div className="bg-[#E5DDD5] p-3 rounded-lg mt-2">
                                                        ...
                                                        {(tpl.buttons || []).length > 0 && (
                                                            <div className="mt-1 space-y-0.5 max-w-[90%]">
                    ...
                    <DialogContent className="max-w-lg max-h-[90vh] overflow-y-auto">
                        ...
                        {mappingTemplate && (
                            <div className="space-y-4">
                                ...
                                {(mappingTemplate.buttons || []).map((btn, idx) => (
                                    <div key={idx} className="bg-white rounded-lg mx-3 mb-1 px-3 py-2 flex items-center justify-center gap-2 shadow-sm">
                                        <ExternalLink className="w-4 h-4 text-[#0096DE]" />
                                        <span className="text-sm font-medium text-[#0096DE]">{btn.text || btn.button_text || `Button ${idx+1}`}</span>
                                    </div>
                                ))}
                                </div></div>
                                <div className="space-y-3">
                                ...
                                {mappingTemplate.buttonVars?.length > 0 && (
                                    <div className="mt-4 pt-4 border-t border-gray-200">
                                        ...
                                        {(mappingTemplate.buttons || []).filter(b => b.url_type !== "dynamic").map((btn, i) => (
                                            <p key={`static-${i}`} className="text-xs text-gray-400 mt-1">"{btn.text}" — {btn.type === "URL" ? "static URL" : btn.type} (no mapping needed)</p>
                                        ))}
                                    </div>
                                )}
...
{`{
  ...
  Object.entries(labelsData).filter(([, v]) => v).map(([, label]) => `\n  "${label}": "value"`).join(",")
                ...
                {/* CR-036 B.3 (E-B3-12): inline media re-upload modal */}
                <Dialog open={!!reuploadTemplate} onOpenChange={(o) => !o && setReuploadTemplate(null)}>
                    <DialogContent data-testid="media-reupload-modal">
                        <DialogHeader>
                            <DialogTitle>Re-upload media — {reuploadTemplate?.template_name}</DialogTitle>
                            <DialogDescription>
                                Attach a new {reuploadTemplate?.header_type} file. The template stays approved — only the media is replaced.
                            </DialogDescription>
                        </DialogHeader>
                        {reuploadTemplate && (
                            <MediaHeaderUpload
