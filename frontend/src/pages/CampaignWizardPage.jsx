...
const CampaignWizardContent = () => {
    ...
    const [searchParams] = useSearchParams();    // CR-076 E-E
    ...
    // CR-076 E-E: Pre-fill audience from lifecycle page navigation (?audience_stage=churned)
    useEffect(() => {
        const audienceStage = searchParams.get("audience_stage");
        const audienceCountParam = parseInt(searchParams.get("audience_count") || "0");
        if (audienceStage && audienceStage !== "all" && !campaignId) {
            const STAGE_LABELS = {
            ...
            setAudienceCount(audienceCountParam);
        }
    ...
    useEffect(() => {
        // CR-076: if URL has audience_stage, skip on every run — prefill effect owns name/count
        const audienceStage = searchParams.get("audience_stage");
        if (audienceStage && audienceStage !== "all" && !campaignId) return;
        if (audienceId.startsWith("lifecycle_stage:")) return;   // belt-and-suspenders guard
                    ...
                    <div className="mb-4">
                        ...
                        <Select value={audienceId} onValueChange={setAudienceId} disabled={isScheduleLocked}>
                            ...
                            <SelectContent>
                                ...
                                {/* CR-076: show lifecycle stage audience when pre-filled from URL param */}
                                {audienceId.startsWith("lifecycle_stage:") && (
                                    <SelectItem value={audienceId}>{audienceName} ({audienceCount.toLocaleString()} customers)</SelectItem>
                                )}
                                {segments.map(s => (
