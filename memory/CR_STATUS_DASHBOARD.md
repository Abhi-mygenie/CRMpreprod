...
| **041** | **Webhook Timestamp Overwrite on transition_ignored (P1 HIGH, pre-existing)** | **📋 Registered — Discovery + Impact Analysis + Planning complete, awaits owner Q1-Q4** | **📋** | ~15 min (~10 LOC, 1 file re-ordering) | **Silent corruption of `delivered_at` / `read_at` / `rejected_at` on every duplicate or out-of-order webhook.** Bug at `routers/whatsapp.py:1454-1469` assigns timestamps BEFORE state-machine gate at line 1472. Result: even when transition is correctly rejected (verdict `transition_ignored`), the timestamp field still gets overwritten with the retry's `ts_utc_iso`, losing the original event time. **Blast-radius (live prod scan)**: 1 row detected with obvious corruption (`phone=7505242126`, `status=rejected` but `delivered_at=15:12` set 6h AFTER `rejected_at=08:50`) plus 1 more with `delivered_at > rejected_at`; invisible corruption (delivered_at → later delivered_at retry) likely much larger. **Impact**: MessageStatusPage relative-time display wrong on retries; future analytics inherit bad data. **Discovered by**: QA agent Test 11 during CR-039 regression. **Fix Option A**: move timestamp block AFTER state-machine gate, gate by `applied`. Pure statement re-ordering, zero logic change. Existing `tests/test_cr039_webhook.py::TestDuplicateDelivered` currently FAILS, will PASS post-fix. **Non-goals**: state machine unchanged, frontend unchanged, no schema migration, no backfill in this CR. Discovery: `discovery/CR_041_TIMESTAMP_OVERWRITE_DISCOVERY.md`. Impact: `planning/CR_041_TIMESTAMP_OVERWRITE_IMPACT_ANALYSIS.md`. Plan: `planning/CR_041_TIMESTAMP_OVERWRITE_PLAN.md`. Follow-ups: CR-041-B (retroactive audit script), F1 (split file), F2 (compound unique index), F3 (enable AuthKey HMAC). | **2026-07-03** |
...
| Date | CR | From → To |
|---|---|---|
| 2026-07-03 | **CR-041** | — → **📋 REGISTERED · DISCOVERY + IMPACT ANALYSIS + PLANNING COMPLETE · P1 HIGH**. Pre-existing bug surfaced by QA (Test 11 failure) during CR-039 regression. `routers/whatsapp.py:1454-1469` assigns `delivered_at`/`read_at`/`rejected_at`/`failure_reason` BEFORE the state-machine gate at line 1472. Result: silently overwrites original event timestamps whenever webhook is `transition_ignored` (duplicate delivered, late delivered on read/rejected, etc.). Blast-radius scan of live prod DB: 1 row with obvious corruption (phone 7505242126 status=rejected but delivered_at 6h AFTER rejected_at), estimated invisible corruption much larger. Impact: MessageStatusPage.jsx:610-612 shows wrong "Delivered X ago" times; future analytics inherit bad data. All read paths enumerated (frontend only surface today). Fix Option A: re-order timestamp block AFTER state-machine gate, ~10 LOC in 1 file, 15 min effort, LOW risk (pure re-ordering). Existing pytest test already covers regression — will flip from FAIL to PASS. Awaits owner Q1-Q4 answers. |
