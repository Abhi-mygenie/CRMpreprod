...
| **030** | **Freshmarketer Webhook Endpoint (`POST /api/pos/webhook`)** | **🔵 Planning approved — ready to implement** | **🔵** | ~2.5 hrs | Freshmarketer sends nested envelope: `{Body: {event, id, data: {contact, event_details, custom_data}}}`. Extract `custom_data` for our params, int→str coercion, contact fallback, idempotency on `Body.id`, new `webhook_logs` collection, reuse DirectSend send logic. Discovery: `discovery/CR_030_*.md`. Plan: `planning/CR_030_*.md`. | **2026-06-26** |
