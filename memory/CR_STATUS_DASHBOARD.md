...
| **030** | **Freshmarketer Webhook Endpoint (`POST /api/pos/webhook`)** | **📋 Registered — discovery in progress** | **📋** | ~½ day | Freshmarketer sends WhatsApp trigger webhooks with nested envelope: `{event_type, data: {contact, event_details, custom_data}}`. Our CR-DIRECT-SEND params (`mobile`, `country_code`, `template_id`, named vars) live in `data.custom_data`. New `POST /api/pos/webhook` needed — unwraps envelope, extracts `custom_data`, falls back to `contact` fields, logs full raw webhook, reuses `/api/pos/send` core logic. Discovery: `discovery/CR_030_FRESHMARKETER_WEBHOOK_ENDPOINT_DISCOVERY.md`. | **2026-06-26** |
