...
| **036** | **Media Header Upload for Template Builder (Image/Video)** | **📋 Intake complete · defaults locked per Meta v21 spec · ready for Planning to validate** | **📋** | ~5.5 hrs | Templates with image/video headers are REJECTED by Meta because CRM sends raw URL as `header_handle`. Meta v17+ requires a pre-uploaded media handle from `POST /v21.0/{WABA_ID}/uploads`. Needs: (1) backend proxy upload endpoint, (2) file picker in Template Builder replacing URL input, (3) handle wired into template creation. **Owner answer (2026-07-03)**: "as per meta standard we have been following that in template builder" → interpreted as: **Q1** all four types (image/video/document/audio), **Q2** Meta size caps (img 5MB, video 16MB, doc 100MB, audio 16MB), **Q3** Meta pass-through via `/uploads` handle (~30d validity). Defaults locked pending Planning confirmation. Discovery: `discovery/CR_036_MEDIA_HEADER_UPLOAD_DISCOVERY.md`. Answers: `discovery/SESSION_2026_07_03_BATCH_INTAKE.md` §CR-036. | **2026-07-01** |
...
| **042** | **Download Message Report per Campaign / Event** | **📋 Intake complete · ALL Q1-Q4 answered · ready for Planning** | **📋** | ~3-5 hrs | **Q1**: BOTH entry-points (MessageStatusPage filter-aware + CampaignHistoryPage per-run). **Q2**: BOTH formats (CSV + XLSX, CR-035 dropdown pattern). **Q3**: proposed 12-field list accepted (sent_at, phone, name, event/campaign, template, status, delivered_at, read_at, rejected_at, error, message_id, is_test). **Q4**: 5000-row cap (matches CR-035). Intake + answers: `discovery/SESSION_2026_07_03_BATCH_INTAKE.md`. **Blocked on**: Planning agent invocation. | **2026-07-03** |
| **043** | **Filter by Customer Tags on Customers page + tag-popover UX rework** | **📋 Intake complete · scope locked (Customers page only) · popover UX spec TBD at Planning** | **📋** | ~3-5 hrs | Scope narrowed by owner to CustomersPage only. Two parts: (a) add tag filter dimension (backend already filters by `tags`; frontend UI missing), (b) rework the inline tag add/edit popover — visual spec TBD (screenshot attached showed BUG-009 context, not the popover). **Deferred to future CR**: tag filter on MessageStatusPage + other surfaces. Intake + answers: `discovery/SESSION_2026_07_03_BATCH_INTAKE.md`. **Blocked on**: Planning agent + owner popover UX spec. | **2026-07-03** |
| **BUG-009** | **"Details" button on Marketing > History does nothing** | **🔴 OPEN · Intake complete · recommendation on file (option a — deep-link to filtered Messages)** | **🔴** | ~15-30 min | Dead onClick on `CampaignHistoryPage.jsx:164-166`. **Recommendation**: navigate to `/messages?campaign_id=X&run_id=Y` extending CR-026 URL scheme (cheapest, ~15 LOC, matches existing pattern). Rationale in intake doc. **Blocked on**: Planning agent to confirm + dispatch to Bug Fix role. | **2026-07-03** |
