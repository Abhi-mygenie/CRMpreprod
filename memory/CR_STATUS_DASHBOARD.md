...
| **036** | **Media Header Upload for Template Builder (Image/Video)** | **📋 Registered — Discovery complete, awaits Q1-Q3 + planning** | **📋** | ~5.5 hrs | Templates with image/video headers are REJECTED by Meta because CRM sends raw URL as `header_handle`. Meta v17+ requires a pre-uploaded media handle from `POST /v21.0/{WABA_ID}/uploads`. Needs: (1) backend proxy upload endpoint, (2) file picker in Template Builder replacing URL input, (3) handle wired into template creation. **Open Q1-Q3:** media types (image only vs video/doc?), max file size, permanent vs pass-through storage. Discovery: `discovery/CR_036_MEDIA_HEADER_UPLOAD_DISCOVERY.md`. | **2026-07-01** |
...
| **042** | **Download Message Report per Campaign / Event** | **📋 Registered — Intake complete, awaits owner Q1-Q4** | **📋** | ~3-5 hrs | Owner wants a downloadable report of WhatsApp message sends per campaign / event (CSV or xlsx). No such endpoint or button exists today. **Open Q1-Q4**: entry-point (MessageStatusPage / CampaignHistoryPage / both), file format (CSV / xlsx / both), fields, row limit. Intake: `discovery/SESSION_2026_07_03_BATCH_INTAKE.md`. | **2026-07-03** |
| **043** | **Filter by Customer Tags / Labels (extension of CR-034)** | **📋 Registered — Intake complete, awaits owner Q1 (BLOCKING)** | **📋** | ~2-7 hrs (Q1-dependent) | CR-034 added tag filter on AudiencesPage only. Owner wants tag filter on OTHER filter surface(s) — most likely CustomersPage and/or MessageStatusPage, but owner report ambiguous. **Open Q1 (BLOCKING)**: which page? **Q2**: (if Message Status) dynamic join vs snapshot on write. **Q3**: ANY / ALL / toggle. Intake: `discovery/SESSION_2026_07_03_BATCH_INTAKE.md`. | **2026-07-03** |
| **BUG-009** | **"Details" button on Marketing > History does nothing** | **🔴 OPEN — Intake complete, awaits owner Q1** | **🔴** | ~15-30 min | `CampaignHistoryPage.jsx:164-166` — button has no `onClick`. Dead UX element. Sibling "Resend" button on same row is wired correctly (line 168+). **Open Q1**: what should Details show? (a) deep-link to Messages filtered by run_id, (b) inline modal with run stats, (c) both, (d) dedicated run detail page. Intake: `discovery/SESSION_2026_07_03_BATCH_INTAKE.md`. | **2026-07-03** |
