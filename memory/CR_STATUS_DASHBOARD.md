...
| **039** | **🚨 Webhook Row Disambiguation (Composite-Key Lookup)** — P1 CRITICAL | **📋 Registered — Discovery + Planning complete, awaits owner Q1-Q4** | **📋** | ~1 hour (~20 LOC, 1 file) | **Silent data corruption in every multi-recipient campaign.** AuthKey returns duplicate `LogID` values for concurrent single-mobile sends, so N rows in `whatsapp_message_logs` share the same `message_id`. Webhook handler at `routers/whatsapp.py:1399` uses `find_one({"message_id": logid})` → always returns first-inserted row → rows 2..N stay `pending` forever even though actually delivered. Mismatch handler at 1426 detects but does not block wrong-row update. **Reproduced live** in prod DB on 2026-07-03 Jeh's Nest campaign (3 recipients, 3 rows sharing logid `d1cbdc206ce89f7f794575bbd862a27b`; 2 backend `mobile mismatch` warnings). **Fix Option A**: composite `(message_id, customer_phone)` lookup with fallback + skip on persistent mismatch (verdict `ambiguous_row`). **Open Q1-Q4**: approve Option A? backfill 2 stuck rows? register CR-040 for AuthKey escalation? verdict marker? Discovery: `discovery/CR_039_WEBHOOK_ROW_DISAMBIGUATION_DISCOVERY.md`. Plan: `planning/CR_039_WEBHOOK_ROW_DISAMBIGUATION_PLAN.md`. Source: INV-2026-07-03-04. | **2026-07-03** |
...
| Date | CR | From → To |
|---|---|---|
| 2026-07-03 | **CR-039** | — → **📋 REGISTERED · DISCOVERY + PLANNING COMPLETE · P1 CRITICAL**. Live-reproduced during Jeh's Nest (restaurant 635) real campaign send. AuthKey `requestjson.php` returns identical `LogID` for concurrent single-mobile sends → 3 rows in `whatsapp_message_logs` share `message_id=d1cbdc206ce89f7f794575bbd862a27b`. Webhook `find_one({"message_id": logid})` at `routers/whatsapp.py:1399` always resolves to first-inserted row → rows 2..N stuck at `pending` even though webhook confirmed delivery. Mismatch handler at line 1426 flags but doesn't block wrong-row update. **Two backend warnings** confirmed at 08:48:17 and 08:50:33. Fix: composite `(message_id, customer_phone)` lookup with legacy fallback + `verdict="ambiguous_row"` skip. ~20 LOC in 1 file. **BLOCKED** on owner Q1-Q4 approval. Discovery + Planning docs in place. |
