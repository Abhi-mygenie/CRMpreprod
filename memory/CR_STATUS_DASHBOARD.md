...
|---|---|---|---|
| 1 | **CR-014** | 🟡 QA passed | Awaiting POS team for hotel folio `room_info` fields |
| 2 | **CR-023** | 🟡 QA passed | Awaiting owner E2E test (Meta template submission) |
| 3 | **CR-026** | 📋 Registered | Campaign "View Messages" deep-link (~½ day) |
...
| **033** | **Additional Audience Filters — enumerate + wire up** | **🔵 Discovery complete — awaits owner priority + Q1-Q6** | **🔵** | ~1.5 days for MVP (P0+P1+P2 cheap) | 30 additional filter dimensions proposed across 4 tiers. **P0 (~1 hr, fixes BUG-A)**: vip_flag, has_birthday_this_month, whatsapp_opt_in, is_blocked, blacklist_flag, complaint_flag. **P1 (~4 hrs, existing data)**: anniversary-this-month, birthday-in-specific-month, age bracket, gender, signup-recency, lead_source, has-GST, has-notes, wallet balance, total coupons, points earned. **P2 (~1 day, join-based)**: campaign attribution, WhatsApp status, last order date, avg order value, order type, payment method, item/category ordered. **P3 (deferred)**: 8 dimensions blocked by empty data. Blocking Qs: AND/OR combinator (recommend dim-level OR / cross-dim AND), UI grouping (Collapsible sections), cached-fields vs live-aggregation. Zero hotspot files. Discovery: `discovery/CR_033_ADDITIONAL_AUDIENCE_FILTERS_DISCOVERY.md`. | **2026-07-01** |
| **034** | **Customer Tag System (free-form tags + tag-based audience filter)** | **🔵 Intake complete — awaits Q1-Q6 + planning approval** | **🔵** | ~8-10 hrs | Approach A (embedded): add `Customer.tags: List[str]` + `users.available_tags` catalog. 5 new endpoints (list/add/remove/bulk-tag/bulk-untag). Extend `build_customer_query` with a `tags` filter (OR default per recommended Q2). UI: tag chips + inline input on CustomersPage rows + bulk action on multi-select + new tag filter chip on AudiencesPage. Backfill: auto-add `VIP` tag to the 46 customers with `vip_flag=true`. Zero hotspot files. Zero DB migration (Mongo). Companion to CR-033 (independent tracks). Intake: `discovery/CR_034_CUSTOMER_TAG_SYSTEM_INTAKE.md`. | **2026-07-01** |
