...
> **⚠️ SUPERSEDED 2026-05-26.** The "INCONCLUSIVE" verdict below is obsolete. POS team has since shipped the contract fixes; CRM has committed **76 redeem `points_transactions` rows totalling 8,633 points** on R689 (Kunafa Mahal), with the latest redeem at 2026-05-26 05:16:06 UTC. The flow is fully verified live.
>
> **Closure doc:** `qa/CR_001C_LR_REALTIME_ORDER_REDEMPTION_CLOSURE_2026_05_26.md`
> **Closure status:** `cr001c_lr_realtime_order_redemption_verified`
>
> This file is preserved for history. The 2026-05-25 `payment_status` gate removal note (immediately below) remains accurate.

> **UPDATE — 2026-05-25** — The `payment_status` gate in `_validate_order` (`backend/routers/pos.py`) has been **REMOVED**. CRM now accepts orders regardless of `payment_status` (no longer requires `"success"`). Tracker: `cr001c_lr_payment_status_gate_removed` in `CR_001_INDEX.md`. The `payment_status: n/a` cell in §below remains accurate for what POS sent; CRM's acceptance criteria no longer require this field.
