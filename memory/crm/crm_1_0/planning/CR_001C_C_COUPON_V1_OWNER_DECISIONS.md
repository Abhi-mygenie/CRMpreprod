...
| Q5 | Discount basis & source of truth | **C** | POS applies coupon locally and sends actual `coupon_code` + `coupon_discount` + `order_total` in final payload. CRM validates/records based on POS-sent values. Coupon is treated as a POS discount, same final-commit pattern as Loyalty. | Owner directive: coupon mirrors loyalty redeem flow — POS is billing source of truth, CRM commits at final payload. | CRM validates coupon at final order, records usage with POS-sent discount amount, may cross-check but does not override POS. |
...
## 9. Q5 — Coupon Discount Basis & Source of Truth

**Question:** Where does the coupon discount get computed and which value does CRM trust?

| Option | Description |
|---|---|
| A | CRM computes from pre-tax subtotal it derives itself |
| B | CRM computes from post-tax total it derives itself |
| **C (Selected)** | POS applies coupon locally and sends actual discount amount + coupon code + order_total in final payload. CRM validates and records using POS-sent values. Same final-commit pattern as Loyalty. |
...
| Q5 | Source of truth = POS sends actual coupon_code + coupon_discount + order_total; CRM validates/records on final payload (same pattern as loyalty) | FROZEN |
