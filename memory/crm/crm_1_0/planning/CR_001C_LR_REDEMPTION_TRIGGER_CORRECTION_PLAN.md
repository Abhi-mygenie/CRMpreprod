...
| Payment | `payment_method`, `payment_status`, `payment_type`, `transaction_id` | **Update 2026-05-25 — `payment_status` no longer gates order acceptance.** The `_validate_order` block in `pos.py` that rejected orders unless `payment_status == "success"` has been removed. Value is now stored as-sent (`"paid"`, `"pending"`, `null`, etc. all accepted). See PRD.md and `CR_001_INDEX.md` row `cr001c_lr_payment_status_gate_removed`. |
