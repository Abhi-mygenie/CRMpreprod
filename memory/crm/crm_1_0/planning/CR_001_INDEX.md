...
| **LR R689 Realtime Investigations (read-only)** | 🟥 **`cr001c_lr_r689_realtime_payload_missing_loyalty_fields`** + sub-finding `cr001c_lr_r689_realtime_payload_not_received` | **2026-05-24** | 7 R689 test orders today (`868917`, `868924`, `868925`, `868928`/prepaid, `868929`/postpaid, `868931`, `008976`) — all missing from `pos_request_logs` AND `orders` collection. Last landed R689 order is still `868908` from 2026-05-23 11:38 UTC. POS IS hitting CRM today on read endpoints (`/api/pos/customer-lookup`, `/api/pos/max-redeemable`, `/api/pos/customers?search`) but **never** on `/api/pos/orders` (zero 4xx either — outbound call silently absent, not failing). Customer counters unchanged. CRM-side ready (52/52 QA, alias acceptance live, 5 callers wired) — ball in POS team's court to wire bill-finalize action. See `analysis/CR_001C_LR_R689_*.md` (4 reports). |
...
| **BLOCKER — Final Realtime Redemption QA** | 🟧 **`cr001c_loyalty_waiting_pos_loyalty_points_key_for_final_realtime_redemption_qa`** | **2026-05-24** | CRM-side complete (52/52 LR + 17/17 L4). POS must send `used_loyalty_point` / `loyalty_points_used` + actual `order_amount` (not 0) in final `POST /api/pos/orders` payload. Without this key, CRM cannot trigger redemption. Multiple R689 test orders (868928, 868932, 868933, 868934, 868935) landed in CRM with `order_amount=0` and zero loyalty fields. See `qa/CR_001C_LR_REALTIME_ORDER_REDEMPTION_VERIFICATION_REPORT.md`. Once POS sends the key, run CR-001C-LR Realtime Order Redemption Verification. Target: `cr001c_lr_realtime_order_redemption_verified`. |
