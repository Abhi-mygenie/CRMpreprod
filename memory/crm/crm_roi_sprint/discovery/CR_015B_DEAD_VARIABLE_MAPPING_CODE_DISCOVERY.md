...
**Lifecycle stage**: `cr015b_removal_DONE` (owner approved B2 + Segments leftovers, 2026-05-29)
**Access used**: read-only static analysis (grep + file reads) → removal executed

---

## ✅ REMOVAL COMPLETED — 2026-05-29

Owner approved **B2 (full coupon-cluster removal) + Segments leftovers**. Executed:

**`components/shared/WhatsAppAutomationContent.jsx`:**
- Removed orphaned state: `showVariableMappingModal`, `mappingTemplate`, `variableMappings`,
...
- Removed orphaned functions: `fetchCouponSummary`, `isCouponVariable`, `parseCouponPickMapping`,
  `getSelectedCoupon`, `handleCouponSelect`, `getCouponPickPreviewValue`,
  `openVariableMappingModal`, `handleSaveVariableMapping`.
- Removed the entire `{/* Variable Mapping Modal */}` `<Dialog>` JSX (~296 lines).
- Simplified the `coupon_pick` branch in `resolvePreviewWithSampleData` (now falls through to
  `sampleCustomerData` lookup — identical behavior, coupons were never fetched on this page).
- Removed now-unused `Tag` icon import.

**`pages/SegmentsPage.jsx`:**
- Removed unused `availableFields` array (L76–91).
- Removed unused `getPreviewMessage()` (legacy `{{name}}`/`{{points}}` preview).

**KEPT (live, untouched):** `templateVariableMappings`/`...Modes` (read-only previews + select
filter), `resolvePreviewWithSampleData`, `sampleCustomerData`, `availableVariables`,
`TestTemplateModal`, `previewTemplate`/`showTemplatePreview`, Segments `templateVariables`
(send payload) + inline preview (L922–957). **TemplatesPage.jsx untouched** (the one live mapping surface).

**Verification:**
- `yarn lint` (eslint) → clean on both files.
- webpack → compiled (only pre-existing `react-hooks/exhaustive-deps` warnings).
- `grep` for all removed symbols → **zero residual references**.
- All 3 routes (`/whatsapp-automation`, `/segments`, `/templates`) load the React bundle with
  **no runtime JS errors** (redirect to /login via ProtectedRoute = expected; no `is not defined`).
- ⚠️ Logged-in visual screenshot NOT captured: `/api/auth/demo-login` returns **404**
  (pre-existing, unrelated bug) and no test password available. Owner to confirm visually,
  or provide a test login for a logged-in screenshot.
