...
Eleven separate, related CRs (was six at sprint start; CR-006, Hotfix, CR-007, CR-008, CR-009, CR-010 added later; CR-011, CR-012 added 2026-05-28; CR-015a sub-CR added 2026-05-29). They must **NOT** be merged into one.
...
| 18 | `CR-015a Preview Sample Data Gap for T5 Variables` | Sub-CR of CR-015. Template preview shows "NA" for 14 new T5 variables because `GET /api/customers/sample-data` endpoint hardcodes only original 23 keys. Fix: add 14 keys to backend + frontend fallback to registry `example` field. ~22 LoC. | **Discovery:** `../discovery/CR_015A_PREVIEW_SAMPLE_DATA_GAP_DISCOVERY.md` ⏸ Awaiting owner approval of fix approach (Option A+B recommended) | `cr015a_discovery_complete_awaiting_approval` |
