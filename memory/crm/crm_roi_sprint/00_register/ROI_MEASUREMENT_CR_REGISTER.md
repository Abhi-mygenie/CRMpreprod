...
| 21 | `CR-017 /pos/max-redeemable Missing Projected Points Earned` | Hot production fix. POS cannot show "you'll earn X points" before payment — `projected_points_earned` + `projected_earn_percent` missing from `/pos/max-redeemable` response. Earn info only available post-payment in `/pos/orders`. Fix: add 2 additive fields using existing `calculate_points()`. Single endpoint, ~10 LoC, non-breaking. | **Discovery:** `../discovery/CR_017_MAX_REDEEMABLE_PROJECTED_POINTS_EARNED_DISCOVERY.md` ⏸ awaiting owner approval | `cr017_discovery_complete_awaiting_owner_approval` |
