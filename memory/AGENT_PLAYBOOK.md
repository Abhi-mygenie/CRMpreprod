...
## 11. POS field alias consistency check

When adding a field that POS sends under one name but CRM stores under another, ensure BOTH models accept the alias:

| POS sends | Order webhook (`OrderItem`) | Validate (`POSCartItem`) | Coupon engine expects |
|---|---|---|---|
| `item_id` | → `pos_food_id` (alias) → cart_dicts `food_id` | → `food_id` (alias) → model_dump `food_id` | `food_id` |
| `item_category` | → `item_category` (direct) → cart_dicts `category_id` + `item_category` | → `category_id` (alias) + `item_category` → model_dump both | `category_id` or `item_category` fallback |

**Rule**: if a POS field name is accepted by `OrderItem` aliases, it must also be accepted by `POSCartItem` aliases. Otherwise the validate endpoint and the order webhook will match items differently.

**Verify**: after adding an alias, test both:
```bash
# Validate endpoint (POSCartItem)
curl -X POST .../api/pos/coupons/validate -d '{"items": [{"item_id": "201951", ...}]}'
# Order webhook (OrderItem → cart_dicts)
curl -X POST .../api/pos/orders -d '{"items": [{"item_id": "201951", ...}]}'
```
