...
**Status**: 🔴 OPEN — 🔵 IMPL GATE OPEN (plan `crm/crm_roi_sprint/planning/BATCH_2026_07_12_IMPL_PLAN.md` §B — read-time aggregation, NO backfill, webhook untouched per owner lock)
...
**Status**: 🔴 OPEN — 🔵 IMPL GATE OPEN (plan `crm/crm_roi_sprint/planning/BATCH_2026_07_12_IMPL_PLAN.md` §A — lazy filter init + last-request-wins, owner locked)
**Severity**: P2 · **Risk**: LOW-MEDIUM (frontend-only fix locked)
...
**Root-cause hypothesis (unconfirmed)**: mount-time double-fetch race in `MessageStatusPage.jsx` — fetch effect (`:188-191`, deps `[filters, pagination.skip]`) fires once with default `campaign_id:"all"` and again after URL-param `setFilters` (`:113-125`); if the unfiltered response resolves last it overwrites `logs`/`total`. Backend `_build_message_log_query` (whatsapp.py:1464-1481) not suspect — manual dropdown filtering works.
**Candidate fixes**: lazy-init filters from `searchParams` / guard first fetch / last-request-wins (AbortController).
...
**Reported**: 2026-08-04  
**Severity**: P1 · **Risk**: MEDIUM · **Status**: 🔴 OPEN  
