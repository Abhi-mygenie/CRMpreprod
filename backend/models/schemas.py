...
class CustomerBase(BaseModel):
    ...
    # BUG-011: default True — manually-added customers were silently opted out
...
class Customer(BaseModel):
    ...
    # Contact & Marketing Permissions
    # BUG-011: default True (see CustomerCreate)
    whatsapp_opt_in: bool = True
    whatsapp_opt_in_date: Optional[str] = None
    promo_whatsapp_allowed: bool = True
    promo_sms_allowed: bool = True
...
class LoyaltySettings(BaseModel):
    ...
    at_risk_days_start: int = 31       # last_visit > N-1 days = At Risk begins (default 30)
    at_risk_days_end: int = 60         # last_visit > N days = Dormant begins (default 60)
    dormant_days_end: int = 90         # last_visit > N days = Churned begins (default 90)
    new_customer_max_visits: int = 1   # visits <= N = "New" not "Active" (default 1)
