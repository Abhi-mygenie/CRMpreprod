...
async def pos_order_webhook(
    ...
    try:
        ...
        # 3b. CR-001C-LR correction (2026-05-23): if POS embedded a loyalty
        # redemption in the order payload, commit it BEFORE the earn calc so
        # earn is computed on the net amount (Q-CORR-3 frozen: Option B).
        # On any non-success code other than auto-cap, hard-fail the entire
        # order webhook (Q-CORR-2 frozen: Option C).
        ...
        if order_data.loyalty_points_used and order_data.loyalty_points_used > 0:
            idem_key = (
                ...
                or f"order_{order_data.order_id}"  # Q-CORR-4 frozen: Option A
            )
            loyalty_redeem_result = await redeem_loyalty_points(
                ...
                customer=customer,
                settings=settings,
                points_to_redeem=int(order_data.loyalty_points_used),
                order_id=order_data.order_id,
                order_total=order_data.order_amount,
                idempotency_key=idem_key,
            )
            if not loyalty_redeem_result["ok"]:
                # Hard-fail order. Non-success indicates POS-side calc
                # diverged from CRM truth.
                return POSResponse(
                    success=False,
                    message=f"Loyalty redemption failed: {loyalty_redeem_result['message']}",
                    data=loyalty_redeem_result["data"],
                )
            loyalty_redeemed_value = float(loyalty_redeem_result["data"].get("redeemed_value", 0))
            # Refresh customer doc with post-redeem balances so the
            # subsequent earn/customer-update steps use accurate state.
