...
# CR-077: DAILY_LIMIT moved to per-tenant loyalty_settings.campaign_daily_limit (default 1000)
...
# ── BUG-011 helpers (read-time aggregation of run stats) ────────────────

async def _augment_run_stats(runs: list, user_id: str) -> list:
    """BUG-011: enrich campaign_runs with delivered/read/sent counts by aggregating
    whatsapp_message_logs at read time. No webhook change, no backfill.

    Legacy compat (BUG-006 / CR-055-scan): logs may carry the run id in EITHER
    `reference_id` (new) or `campaign_id` (pre-BUG-006). We match on both via $or.

    Semantics:
      total_sent      = count(status in sent|delivered|read)   — successful provider handoff
      total_failed    = count(status in failed|rejected)       — provider rejected or pre-send skip
      total_delivered = count(status in delivered|read)        — delivered subsumes read
      total_read      = count(status == read)

    Fallback: any run with ZERO matching logs (legacy pre-logging or resend-only)
    keeps its stored `total_sent`/`total_failed` values unchanged. delivered/read
    default to 0 for those runs (no way to derive without logs).
    """
    if not runs:
        return runs
    ...
    pipeline = [
        {"$match": {
            "user_id": user_id,
            # CR-055-scan: legacy compat — match run id on either field
            "$or": [
                {"reference_id": {"$in": run_ids}},
                {"campaign_id": {"$in": run_ids}},
            ],
        }},
        {"$project": {
            # Prefer reference_id (new), fall back to campaign_id (legacy)
            "run_key": {"$ifNull": ["$reference_id", "$campaign_id"]},
            "status": 1,
        }},
        {"$group": {
            "_id": {"run_key": "$run_key", "status": "$status"},
            "count": {"$sum": 1},
        }},
    ]
    ...
    # {run_id: {status: count}}
    by_run: dict = {}
    async for row in db.whatsapp_message_logs.aggregate(pipeline):
        rk = row["_id"]["run_key"]
        st = row["_id"]["status"] or "unknown"
        by_run.setdefault(rk, {})[st] = row["count"]

    for r in runs:
        counts = by_run.get(r.get("id"))
        if not counts:
            # Fallback: no logs found for this run — preserve stored totals.
            r.setdefault("total_delivered", 0)
            r.setdefault("total_read", 0)
            continue
        ...
        r["total_sent"] = sent
        r["total_failed"] = failed
        r["total_delivered"] = delivered + read   # delivered subsumes read
        r["total_read"] = read
    return runs


async def _augment_campaign_stats(campaigns: list, user_id: str) -> list:
    """BUG-011 extension: enrich campaigns with delivered/read/failed counts by
    aggregating whatsapp_message_logs at read time. Same pattern as _augment_run_stats
    but groups by campaign_id instead of run_id.

    Legacy compat (BUG-006 / CR-055-scan): logs may carry the campaign id in EITHER
    `campaign_id` (post-BUG-006) or `reference_id` (pre-BUG-006). Match on both via $or.
    """
    if not campaigns:
        return campaigns
    ...
    pipeline = [
        {"$match": {
            "user_id": user_id,
            # CR-055-scan: legacy compat
            "$or": [
                {"campaign_id": {"$in": camp_ids}},
                {"reference_id": {"$in": camp_ids}},
            ],
        }},
        {"$project": {
            # Resolve which campaign this log belongs to:
            # If campaign_id is one of the campaign ids -> use it; else use reference_id
            "camp_key": {"$cond": {
                "if": {"$in": ["$campaign_id", camp_ids]},
                "then": "$campaign_id",
                "else": "$reference_id",
            }},
            "status": 1,
        ...
        {"$group": {
            "_id": {"camp_key": "$camp_key", "status": "$status"},
            "count": {"$sum": 1},
        }},
    ]
    ...
    async for row in db.whatsapp_message_logs.aggregate(pipeline):
        ck = row["_id"]["camp_key"]
        st = row["_id"]["status"] or "unknown"
        by_camp.setdefault(ck, {})[st] = row["count"]

    for c in campaigns:
        counts = by_camp.get(c.get("id"))
        if not counts:
            c.setdefault("total_delivered", 0)
            c.setdefault("total_read", 0)
            continue
        delivered = counts.get("delivered", 0)
        read = counts.get("read", 0)
        sent = counts.get("sent", 0) + delivered + read
        ...
        c["total_delivered"] = delivered + read
        c["total_read"] = read
    return campaigns
...
async def _resolve_audience_customers(user_id: str, audience_id: str):
    ...
    elif audience_id.startswith("lifecycle_stage:"):   # CR-076 E-E.3
...
async def list_campaigns(user: dict = Depends(get_current_user)):
    ...
    # BUG-011 extension: augment campaign-level stats from message logs
    # (same root cause — total_delivered/total_read/total_failed stored as 0, never updated)
    return await _augment_campaign_stats(campaigns, user["id"])


@router.get("/daily-limit")
async def get_daily_limit(user: dict = Depends(get_current_user)):
    # CR-077: per-tenant daily limit from loyalty_settings
    settings_doc = await db.loyalty_settings.find_one(
...
async def _execute_campaign_send(campaign_id: str, user_id: str):
    ...
    try:
        ...
        # BUG-011: opted-out customers get a visible skip row (no more silent drops)
        _skip_now = datetime.now(timezone.utc).isoformat()
        for cust in customers:
            if cust.get("whatsapp_opt_in") is False:
                await db.whatsapp_message_logs.insert_one({
                    "id": str(uuid.uuid4()),
                    "user_id": user_id,
                    "customer_id": cust.get("id"),
                    "customer_name": cust.get("name"),
                    "customer_phone": (cust.get("phone") or "").replace(" ", "").replace("-", ""),
                    "template_id": campaign.get("template_id"),
                    "template_name": campaign.get("template_name"),
                    "campaign_id": campaign_id,
                    "status": "failed",
                    "status_note": "opted_out",
                    "created_at": _skip_now,
                    "is_test": False,
                })
        ...
        for cust in eligible:
            ...
            if _media_ht and not _media_url:
                await db.whatsapp_message_logs.insert_one({
                    # CR-036 B.2 (E-B2-5): id + customer_name + template_name so
                    # the log row has stable React key, name column populated,
                    # and template column populated. Row expand/checkbox now work.
                    "id": str(uuid.uuid4()),
                    "user_id": user_id,
                    "customer_id": cust.get("id"),
                    "customer_name": cust.get("name"),
...
async def _execute_resend_subset(campaign_id: str, parent_run_id: str, targets: list, user_id: str):
    ...
    try:
        ...
        for t in targets:
            ...
            # CR-036 B.2 (E-B2-5): resolve customer BEFORE G5 check so
            # customer_name is available for the fail-loud log row too.
            cust = cust_by_phone.get(phone, {"name": "", "phone": phone})

            # CR-036 B.1 G5: fail-loud for media templates missing send_media_url
            if _media_ht and not _media_url:
                await db.whatsapp_message_logs.insert_one({
                    # CR-036 B.2 (E-B2-5): parity with immediate-send G5 row shape.
                    "id": str(uuid.uuid4()),
                    "user_id": user_id,
...
async def get_campaign_runs(campaign_id: str, user: dict = Depends(get_current_user)):
    ...
    # BUG-011: read-time aggregation of delivered/read/sent from whatsapp_message_logs
    return await _augment_run_stats(runs, user["id"])
