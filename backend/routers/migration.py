            ...
            while page <= last_page:
                ...
                for i, mygenie_order in enumerate(order_list):
                    ...
                    try:
                        ...
                        if existing_order:
                            # Preserve points_earned and off_peak_bonus on re-sync
                            # to avoid clobbering values calculated during first sync
...
async def sync_orders_from_mygenie(background_tasks: BackgroundTasks, request: Request, user: dict = Depends(get_current_user)):
    """
    ...
    # Get user's MyGenie token (CR-008: header first, DB fallback)
