...
# =============================================================================
# COUPON ANALYTICS (CR-003 Phase 1 + Phase 2)
# =============================================================================


def _time_period_to_date(time_period: str) -> Optional[str]:
    """Convert preset time_period to ISO date string, or None for all-time."""
    if time_period == "all":
        return None
    days_map = {"7d": 7, "30d": 30, "90d": 90}
...
def _resolve_date_range(time_period: str, date_from: Optional[str], date_to: Optional[str]):
    """Resolve date range: custom dates take precedence over preset pills."""
    if date_from or date_to:
        return date_from, date_to
    return _time_period_to_date(time_period), None


def _roi_label(score, uses):
    """Return human-readable ROI label for CSV/PDF."""
    if score is None:
        return ""
    if uses < 3:
        return "Low Data"
...
async def get_coupon_analytics(
    time_period: str = Query("all", description="Time period: 7d, 30d, 90d, all"),
    date_from: Optional[str] = Query(None, description="Custom start date (ISO)"),
    date_to: Optional[str] = Query(None, description="Custom end date (ISO)"),
    user: dict = Depends(get_current_user),
):
    """CR-003: Coupon analytics dashboard — full breakdown.
    Returns coupon stats for the authenticated restaurant owner,
    optionally filtered by time period or custom date range.
    """
    from services.analytics_service import get_coupon_stats
    resolved_from, resolved_to = _resolve_date_range(time_period, date_from, date_to)
    return await get_coupon_stats(user["id"], date_from=resolved_from, date_to=resolved_to)
...
async def get_top_coupons(
    ...
    """CR-003 Phase 2: Per-coupon performance table.
    Returns all coupons for the restaurant with usage stats,
    optionally filtered by time period or custom date range.
    """
    user_id = user["id"]
    resolved_from, resolved_to = _resolve_date_range(time_period, date_from, date_to)

    # 1. All coupons (not date-filtered)
    coupon_docs = await db.coupons.find(
        {"user_id": user_id},
        {"_id": 0, "code": 1, "title": 1, "discount_scope": 1, "offer_type": 1,
         "discount_type": 1, "discount_value": 1, "is_active": 1,
         "usage_limit": 1, "per_user_limit": 1},
    ).to_list(500)
    coupon_map = {c["code"]: c for c in coupon_docs}

    # 2. Realtime usage aggregated by code (date-filtered)
    usage_match = {"user_id": user_id}
    if resolved_from or resolved_to:
        used_at_filter = {}
        if resolved_from:
            used_at_filter["$gte"] = resolved_from
        if resolved_to:
            used_at_filter["$lte"] = resolved_to
        usage_match["used_at"] = used_at_filter
    usage_pipeline = [
        {"$match": usage_match},
        {"$group": {
            "_id": "$coupon_code",
            ...
            "net_revenue": {"$sum": {"$ifNull": ["$order_total", 0]}},
            "last_used": {"$max": "$used_at"},
        }},
    ]
    usage_rows = {r["_id"]: r for r in await db.coupon_usage.aggregate(usage_pipeline).to_list(500)}

    # 3. Legacy usage (date-filtered on created_at)
    legacy_match = {"user_id": user_id}
    if resolved_from or resolved_to:
        created_at_filter = {}
        if resolved_from:
            created_at_filter["$gte"] = resolved_from
        if resolved_to:
            created_at_filter["$lte"] = resolved_to
    ...
    legacy_rows = {r["_id"]: r for r in await db.coupon_transactions.aggregate(legacy_pipeline).to_list(500)}

    # 4. Left-join: every coupon gets a row
    result = []
    for code, c in coupon_map.items():
        u = usage_rows.get(code, {})
        lg = legacy_rows.get(code, {})
        rt_used = u.get("times_used", 0)
        lg_used = lg.get("times_used", 0)
        rt_disc = u.get("total_discount", 0.0)
        lg_disc = lg.get("total_discount", 0.0)
        rt_net = float(u.get("net_revenue") or 0.0)
        # Pick latest last_used from either source
        rt_last = u.get("last_used")
        lg_last = lg.get("last_used")
        last_used = None
        if rt_last and lg_last:
            last_used = max(str(rt_last), str(lg_last))
        else:
            last_used = rt_last or lg_last
        total_disc = round(float(rt_disc) + float(lg_disc), 2)
        gross_rev = round(rt_net + float(rt_disc), 2)
        roi = round(gross_rev / float(rt_disc), 1) if float(rt_disc) > 0 and rt_net > 0 else None
        result.append({
            "code": code,
            "title": c.get("title", ""),
            "discount_scope": c.get("discount_scope", "order"),
            "offer_type": c.get("offer_type", "simple"),
            "discount_type": c.get("discount_type", "flat"),
            "discount_value": c.get("discount_value", 0),
            "is_active": c.get("is_active", True),
            "times_used": rt_used + lg_used,
            "total_discount": total_disc,
            "gross_revenue": gross_rev,
            "roi_score": roi,
            "last_used": str(last_used)[:19] if last_used else None,
            "usage_limit": c.get("usage_limit"),
            "per_user_limit": c.get("per_user_limit"),
        })

    # Sort: times_used desc, then code asc
    result.sort(key=lambda r: (-r["times_used"], r["code"]))

    return {"coupons": result, "total": len(result)}


@router.get("/coupons/export")
async def export_coupon_analytics(
    time_period: str = Query("all", description="Time period: 7d, 30d, 90d, all"),
    date_from: Optional[str] = Query(None, description="Custom start date (ISO)"),
    date_to: Optional[str] = Query(None, description="Custom end date (ISO)"),
    user: dict = Depends(get_current_user),
):
    """CR-003 Phase 3: Export coupon performance as CSV-ready format."""
    top_resp = await get_top_coupons(
        time_period=time_period,
        date_from=date_from,
        date_to=date_to,
        user=user,
    )
    coupons = top_resp["coupons"]

    scope_labels = {"order": "Order-Level", "item": "Item-Level", "category": "Category-Level"}
    offer_labels = {"simple": "Simple", "bogo": "BOGO", "bxg": "Buy X Get Y",
                    "nth_item": "Every Nth", "free_item": "Free Item", "combo": "Combo"}
    ...
    return {
        ...
        "rows": [
            [
                ...
                f"{c['roi_score']}x" if c.get("roi_score") else "",
                _roi_label(c.get("roi_score"), c["times_used"]),
                c.get("last_used") or "Never",
...
@router.get("/coupons/pdf")
async def export_coupon_analytics_pdf(
    time_period: str = Query("all", description="Time period: 7d, 30d, 90d, all"),
    date_from: Optional[str] = Query(None, description="Custom start date (ISO)"),
    date_to: Optional[str] = Query(None, description="Custom end date (ISO)"),
    user: dict = Depends(get_current_user),
):
    """CR-003 Phase 3: Export coupon analytics as a branded PDF report."""
    from services.analytics_service import get_coupon_stats
    from services.pdf_report import CouponAnalyticsPDFReport

    resolved_from, resolved_to = _resolve_date_range(time_period, date_from, date_to)
    stats = await get_coupon_stats(user["id"], date_from=resolved_from, date_to=resolved_to)
    top_resp = await get_top_coupons(
        time_period=time_period, date_from=date_from, date_to=date_to, user=user,
    )
    coupons = top_resp["coupons"]

    # Build period label
    if resolved_from or resolved_to:
        f_str = resolved_from[:10] if resolved_from else "..."
        t_str = resolved_to[:10] if resolved_to else "..."
        period_label = f"{f_str} to {t_str}"
    else:
        label_map = {"7d": "Last 7 Days", "30d": "Last 30 Days", "90d": "Last 90 Days"}
        period_label = label_map.get(time_period, "All Time")

    restaurant_name = user.get("restaurant_name", "Restaurant")
    user_email = user.get("email", "")

    report = CouponAnalyticsPDFReport(
    ...
    filename = f"coupon-analytics-{datetime.now(timezone.utc).strftime('%Y-%m-%d')}.pdf"
...
def get_stage_cutoffs(settings: dict = None) -> dict:  # CR-077: configurable per-tenant
    """Get date cutoffs for lifecycle stages. Reads per-tenant config from settings dict."""
    s = settings or {}
    ...
    return {
        ...
        "new_max_visits":   new_max_v,   # CR-077: added key for classify_customer_stage
    }
...
def classify_customer_stage(customer: dict, cutoffs: dict) -> str:
    ...
    if last_visit >= cutoffs["thirty_days_ago"]:
        if total_visits <= cutoffs.get("new_max_visits", 1):   # CR-077: configurable
            return "new"
        else:
            return "active"
    ...
    pipeline = [
        ...
        {"$addFields": {
            "stage": {
                "$switch": {
                    "branches": [
                        # New: configurable max visits, last visit within active window
                        {
                            "case": {
                                "$and": [
                        ...
                        # Active: 2+ visits (configurable), last visit within active window
                        {
                            "case": {
                                "$and": [
...
async def get_lifecycle_customers(
    ...
    if stage == "new":
        query["total_visits"] = {"$lte": cutoffs["new_max_visits"]}   # CR-077
        query["last_visit"] = {"$gte": cutoffs["thirty_days_ago"]}
    elif stage == "active":
        query["total_visits"] = {"$gte": cutoffs["new_max_visits"] + 1}   # CR-077
        query["last_visit"] = {"$gte": cutoffs["thirty_days_ago"]}
    elif stage == "at_risk":
        query["last_visit"] = {
