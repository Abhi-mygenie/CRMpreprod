...
def _is_placeholder_name(name) -> bool:
    """BUG-022: True if the name is a migration placeholder — safe to overwrite on re-sync."""
...
def _validate_and_classify_row(row: dict, existing_phones: set) -> dict:
    ...
    # BUG-014 (D1, Q-B): parse whatsapp opt-in — both header styles; None = leave unchanged
    raw_opt = (row.get("whatsapp opt-in") or row.get("whatsapp_opt_in") or "").strip().lower()
    if raw_opt in ("yes", "true", "1"):
        opt_in = True
    elif raw_opt in ("no", "false", "0"):
        opt_in = False
    else:
        opt_in = None
...
def _find_duplicate_phones(classified: list) -> Optional[str]:
    """BUG-013 (Q-A=c): 400 detail string if a phone appears in multiple valid rows, else None."""
    seen = {}
    for r in classified:
        if r["status"] == "error":
            continue
        seen.setdefault(r["phone"], []).append(r["row"])
    dups = {p: rws for p, rws in seen.items() if len(rws) > 1}
    if not dups:
        return None
    parts = [f"{p} (rows {', '.join(map(str, rws))})" for p, rws in list(dups.items())[:10]]
    more = f" and {len(dups) - 10} more duplicates" if len(dups) > 10 else ""
    return (f"Duplicate phone numbers found in file: {'; '.join(parts)}{more}. "
            f"Each phone may appear only once — please fix and re-upload.")
...
async def download_import_template(
    ...
    # BUG-014 (Q-C=a): template includes whatsapp_opt_in column
    IMPORT_HEADERS = ["name", "phone", "email", "dob", "city", "address", "tags", "whatsapp_opt_in"]
    SAMPLE_ROWS = [
        ["Priya Sharma",  "9876543210", "priya@example.com", "1990-05-15", "Mumbai", "123 Main St", "VIP, Regular", "Yes"],
...
async def preview_import(
    ...
    # BUG-013 (Q-A=c): preview/commit parity — reject in-file duplicate phones
    dup_detail = _find_duplicate_phones(classified)
    if dup_detail:
...
async def import_customers(
    ...
    errors         = []
    new_tags_seen  = set()
    ops            = []  # BUG-013: collect writes, execute one bulk_write

    classified = [_validate_and_classify_row(raw_row, set(phone_to_doc.keys())) for raw_row in rows]

    # BUG-013 (Q-A=c): reject files containing in-file duplicate phones
    dup_detail = _find_duplicate_phones(classified)
    if dup_detail:
        raise HTTPException(status_code=400, detail=dup_detail)
    ...
    for result in classified:
        ...
        if result["status"] == "update":
            ...
            # BUG-014 (D1): apply opt-in only when file gives an explicit value
            ...
            new_doc = {
                ...
                **payload,
                ...
                "total_visits":    0,
                "total_spent":     0.0,
                # BUG-014 (D2): default True; explicit file value wins
                "whatsapp_opt_in": result["whatsapp_opt_in"] if result.get("whatsapp_opt_in") is not None else True,
                "created_at":      now,
                "updated_at":      now,
            }
    ...
    if ops:
        await db.customers.bulk_write(ops, ordered=False)  # BUG-013: single round trip
