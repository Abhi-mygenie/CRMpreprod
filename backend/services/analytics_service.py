...
async def get_coupon_stats(user_id: str):
    ...
    legacy_used = await db.coupon_transactions.count_documents({"user_id": user_id})
    pipeline_legacy = [
        {"$match": {"user_id": user_id}},
    ...
    realtime_used = await db.coupon_usage.count_documents({"user_id": user_id})
    pipeline_realtime = [
        {"$match": {"user_id": user_id}},
    ...
    return {
        ...
        "breakdown_by_offer_type": await _get_breakdown_by_offer_type(user_id),
        "time_window_usage": await _get_time_window_usage(user_id),
        "bxgy_usage": await _get_bxgy_usage(user_id),
        "nth_item_usage": await _get_nth_item_usage(user_id),
    }


async def _get_breakdown_by_offer_type(user_id: str):
...
async def _get_time_window_usage(user_id: str):
    ...
    used_within_window = await db.coupon_usage.count_documents({
        "user_id": user_id,
...
async def _get_bxgy_usage(user_id: str):
    """CR-001C-C V3-B — additive analytics for BOGO / Buy-X-Get-Y adoption."""
    # bogo_orders + bxg_orders: usage rows by offer_type.
    bogo_orders = await db.coupon_usage.count_documents({
        "user_id": user_id, "offer_type": "bogo",
    })
    bxg_orders = await db.coupon_usage.count_documents({
        "user_id": user_id, "offer_type": "bxg",
    })
    # Aggregate applied_applications + benefit-unit counts.
    pipeline = [
        {"$match": {
            "user_id": user_id,
            "offer_type": {"$in": ["bogo", "bxg"]},
        }},
        {"$group": {
            "_id": None,
    ...
    cursor = db.coupon_usage.find(
        {"user_id": user_id, "offer_type": {"$in": ["bogo", "bxg"]}},
...
async def _get_nth_item_usage(user_id: str):
    """CR-001C-C V3-C — additive analytics for Every-Nth adoption."""
    orders = await db.coupon_usage.count_documents({
        "user_id": user_id, "offer_type": "nth_item",
    })
    pipeline = [
        {"$match": {"user_id": user_id, "offer_type": "nth_item"}},
        {"$group": {
            "_id": None,
            "total_applications": {"$sum": {"$ifNull": ["$applied_applications", 0]}},
            "discount_amount": {"$sum": {"$ifNull": ["$coupon_discount", 0]}},
