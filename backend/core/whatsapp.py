...
AUTHKEY_API_URL = os.environ['AUTHKEY_API_URL']
...
async def send_single_message(
    ...
    try:
        ...
        # CR-069 FIX B: AuthKey uses "button_param_value" (string) for dynamic URL button suffix
        # Not "buttonValues" (ignored) nor sequential bodyValues (ignored for button URLs)
        # AuthKey API: {"bodyValues": {...}, "button_param_value": "TOKEN"}
        if message.button_values:
            # Take the first button value (AuthKey supports one button_param_value string)
            first_btn_val = next(iter(message.button_values.values()), None)
            if first_btn_val:
                payload["button_param_value"] = first_btn_val

        logger.info(f"Sending WhatsApp to {message.country_code}{message.phone}, template: {message.template_id}")
        logger.info(f"AuthKey payload — bodyValues: {payload.get('bodyValues')} | button_param_value: {payload.get('button_param_value')}")  # DEBUG CR-069
...
def build_body_values(
    ...
    menu_pick_resolved: Dict[str, str] = None,
) -> Dict[str, str]:
    """
    Build the bodyValues dict for AuthKey send.
    For each {{n}}: text mode -> literal; coupon_pick -> resolve from picked coupon;
    menu_pick -> resolve from pre-stored static binding (CR-020);
    (event_data wins for event triggers per D-4); map mode -> resolve via registry.
    """
    body_values = {}
    modes = variable_modes or {}
    event_data = event_data or {}
    menu_pick_resolved = menu_pick_resolved or {}

    for var in template_variables:
        # CR-069: skip button URL vars — they are resolved separately as button_values
        if var.startswith("btn_url_"):
            continue
        ...
        if mode == "text":
            ...
            # CR-020: Static binding — resolved value stored at mapping time
            body_values[var_num] = menu_pick_resolved.get(mapped_field, "")
        else:
...
async def get_event_template_config(db, user_id: str, event_key: str) -> Optional[Dict]:
    ...
    return {
        ...
        "menu_pick_resolved": var_map.get("menu_pick_resolved", {}) if var_map else {},
    }
...
async def trigger_whatsapp_event(
    ...
    try:
        ...
        # CR-020: Load menu_pick_resolved from stored config (static binding)
        menu_pick_resolved = config.get("menu_pick_resolved", {})

        # 4. Build body values via P2 registry resolver
        # CR-069 FIX: exclude btn_url_ keys — they are button URL params, not body variables
        template_variables = [
            k for k in (variable_mappings.keys() if variable_mappings else [])
            if not k.startswith("btn_url_")
        ]
