...
async def validate_coupon_for_customer(
    ...
    *,
    ...
    if channel and channel not in channels:
        return {
            "ok": False,
            "error": {"code": "CHANNEL_NOT_VALID", "field": "applicable_channels", "detail": f"Coupon not valid for channel {channel}"},
        }
...
async def list_available_coupons(
    ...
    *,
    ...
    """
    now_iso = now_iso or _now_iso()
    cursor = db.coupons.find({"user_id": user_id, "is_active": True}, {"_id": 0})
    eligible: list[dict] = []
