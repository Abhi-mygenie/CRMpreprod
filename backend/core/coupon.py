...
def build_display_title(coupon: dict) -> str:
    """Build a human-readable display title for POS UI.
    Examples: 'Buy 1 Get 2 Free', 'Every 3rd Rs.100 off', '10% off'."""
    ot = (coupon.get("offer_type") or "simple").lower()
    if ot in ("bogo", "bxg", "buy_x_get_y"):
        buy_q = int(coupon.get("buy_quantity") or 1)
        get_q = int(coupon.get("get_quantity") or 1)
        gdt = (coupon.get("get_discount_type") or "free").lower()
        gv = coupon.get("get_discount_value")
        if gdt == "free":
            return f"Buy {buy_q} Get {get_q} Free"
        elif gdt == "percentage":
            return f"Buy {buy_q} Get {get_q} at {int(gv) if gv == int(gv) else gv}% off"
        else:
            return f"Buy {buy_q} Get {get_q} Rs.{int(gv) if gv == int(gv) else gv} off"
    if ot in ("nth_item", "every_nth", "every_nth_item"):
        n = int(coupon.get("nth_item_number") or 0)
        bt = (coupon.get("nth_discount_type") or coupon.get("get_discount_type") or "free").lower()
        bv = coupon.get("nth_discount_value") or coupon.get("get_discount_value")
        suffix = {1: "st", 2: "nd", 3: "rd"}.get(n % 10 if n % 100 not in (11, 12, 13) else 0, "th")
        if bt == "free":
            return f"Every {n}{suffix} Free"
        elif bt == "percentage":
            pv = int(bv) if bv and bv == int(bv) else bv
            return f"Every {n}{suffix} {pv}% off"
        else:
            fv = int(bv) if bv and bv == int(bv) else bv
            return f"Every {n}{suffix} Rs.{fv} off"
    # Simple / order / item / category
    dt = (coupon.get("discount_type") or "percentage").lower()
    dv = coupon.get("discount_value") or 0
    fv = int(dv) if dv == int(dv) else dv
    if dt == "percentage":
        return f"{fv}% off"
    return f"Rs.{fv} off"
...
def _v3b_select_get_units(
    ...
    highest = bool(coupon.get("apply_to_highest_item", False))
    # `apply_to_cheapest_item` is functionally redundant — default direction
    # is already cheapest. Kept on the coupon doc for backwards-compatibility
    # but we only branch on `highest` here.
    reverse = highest
...
async def validate_coupon_for_customer(
    ...
    *,
    ...
    if customer_id and per_user_limit is not None and per_user_limit > 0:
...
async def record_coupon_usage_for_order(
    ...
    *,
    ...
    if float(coupon_discount_from_pos or 0.0) == 0.0:
        ...
        # Legacy behavior: ALWAYS skip on POS=0. This hid silent usage-limit
        # loops and let customers re-redeem when POS forgot to apply the
        # discount. Per owner D3 ("if POS sends by mistake CRM shd honour and
        # record drift in log") we defer the skip decision until AFTER
        # validation has produced crm_computed, so we know whether the cart
        # is genuinely eligible. The actual skip-vs-record branch is enforced
        # below at the post-validation late-skip block.
        pos_sent_zero = True
    else:
        pos_sent_zero = False
    ...
    if pos_sent_zero:
        if crm_computed is None or crm_computed <= 0:
            # POS=0 AND CRM=0 → genuinely nothing to record. Skip with warning.
            logger.warning(
            ...
            return {
                ...
                "error": {
                    "code": "INACTIVE", "field": "coupon_discount",
                    "detail": "POS-sent coupon_discount is 0 and CRM-computed is also 0; nothing to record",
                },
            }
        # POS=0 AND CRM>0 → CRM is the safety net. Record using crm_computed,
        # flag drift, log it. Owner D3 verbatim: "if POS sends by mistake CRM
        # shd honour and record drift in log".
        logger.info(
    ...
    # Legacy POS-vs-CRM variance log (POS>0 case) — unchanged behavior.
    if (not pos_sent_zero) and crm_computed is not None and not _within_tolerance(pos_sent, crm_computed):
        logger.warning(
    ...
    # CR-021 D3: mismatch flag — TRUE either when POS=0 with CRM>0 (drift), or
    # when POS>0 differs from CRM beyond tolerance (existing variance case).
    discount_mismatch = bool(
        (pos_sent_zero and crm_computed is not None and crm_computed > 0)
        or (crm_computed is not None and not _within_tolerance(pos_sent, crm_computed))
    )
    ...
    usage_doc = {
        ...
        "coupon_discount": effective_pos_sent,
        ...
        "discount_applied": effective_pos_sent,
        "used_at": now_iso,
        "created_at": now_iso,
        # CR-001C-C V3-A — additive
