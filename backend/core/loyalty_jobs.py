...
async def run_vip_auto_promote(user_id: str, settings: dict) -> dict:
    ...
    meets the configured threshold. Gated by vip_auto_promote_enabled toggle (default OFF).
    ...
    # Restaurant-wide max_spend for normalization (single DB query)
    ...
    for c in customers:
        ...
        days_since = float(dormant_end)   # default to max if no visit data
        if lv_str:
            try:
